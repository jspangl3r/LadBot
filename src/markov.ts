 ...
 */
export function createChain(): Record<string, any> {
  return {};
}
...
export function mergeSentence(
  ...
  for (let i = 0; i < words.length; i++) {
    // Setup some states
    const currentState = `${prepre}:${pre}`;
    const nextState = words[i];
 ...
 */
export function generateSentence(chain: Record<string, any>): string | null {
  // Setup some stuff
  const words = ["<START>", "<START>"];
...
function getNextWord(
  ...
  const currentState = `${words[wordsCount - 2]}:${words[wordsCount - 1]}`;
  const possibilities = chain[currentState];
  ...
  const index = getRandomInt(1, possibilities.count);
  ...
  for (const word in possibilities.next) {
